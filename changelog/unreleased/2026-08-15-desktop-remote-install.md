...
- **A matching Node runtime travels with the image.** The install image is pure JavaScript, so the push fetches the official build for that platform and arch from nodejs.org, verifies it against the release's own `SHASUMS256.txt`, and installs it as `lib/runtime`. The remote needs no Node of its own, and an unverified download never leaves this machine. Runtimes are cached per shape, so the second host of a kind costs no download.
